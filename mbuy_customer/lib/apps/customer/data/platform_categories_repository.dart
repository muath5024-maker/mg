...
class PlatformCategoriesRepository {
  ...
  Future<List<PlatformCategory>> getCategories() async {
    ...
    // Fetch from API
    final response = await _api.getPlatformCategories();
...
final featuredStoresProvider = FutureProvider((ref) async {
  final api = ref.watch(customerApiProvider);
  final response = await api.getFeaturedStores();
  if (response.ok) {
...
final homeBannerStoresProvider = FutureProvider((ref) async {
  final api = ref.watch(customerApiProvider);
  final response = await api.getFeaturedStores(limit: 5);
  if (response.ok) {
    return response.data ?? [];
  }
  return [];
...
final featuredProductsProvider = FutureProvider<List<Product>>((ref) async {
  final api = ref.watch(customerApiProvider);
  final response = await api.getTrendingProducts(limit: 10);
  if (response.ok) {
    return response.data ?? [];
  }
