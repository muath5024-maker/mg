...
// لون primary المستخدم في الشاشة
const Color _primaryColor = AppTheme.primaryColor;
...
class CustomerAccountScreen extends ConsumerWidget {
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            ...
            // Additional Services
            SliverToBoxAdapter(
              child: _buildServicesSection(context, ref, isDark, isGuest),
            ),
  ...
  Widget _buildServicesSection(
    ...
    final services = [
      {'icon': Icons.storefront, 'label': 'كن تاجراً', 'action': 'merchant'},
  ...
  Widget _buildServiceItem(
    ...
    // لون خاص لزر "كن تاجراً"
    final isMerchantAction = action == 'merchant';
    final itemColor = isMerchantAction
    ...
    return GestureDetector(
      onTap: () {
        if (action == 'merchant') {
          // تحويل المستخدم لتاجر
          ref.read(appUserTypeProvider.notifier).state = AppUserType.merchant;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('تم تفعيل حساب التاجر! اذهب لتبويب حسابي'),
      ...
      child: Column(
        ...
        children: [
          Container(
            padding: isMerchantAction ? const EdgeInsets.all(8) : null,
            decoration: isMerchantAction
                ? BoxDecoration(
                    color: const Color(0xFF00BFA5).withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(12),
                  )
                : null,
            child: Icon(icon, size: 28, color: itemColor),
          ),
          const SizedBox(height: 8),
          Text(
            label,
            style: TextStyle(
              fontSize: 11,
              fontWeight: isMerchantAction
                  ? FontWeight.bold
                  : FontWeight.normal,
              color: isMerchantAction
                  ? const Color(0xFF00BFA5)
